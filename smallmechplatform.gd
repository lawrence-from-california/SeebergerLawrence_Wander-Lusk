...
func _process(delta: float) -> void:
	if animation_player:
		...
		if (!animation_player.current_animation_position>=0.99 and animatable_body_2d.animation=="green") or (!animation_player.current_animation_position<=0.01 and animatable_body_2d.animation=="red"):
