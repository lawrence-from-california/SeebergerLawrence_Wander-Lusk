...
@export var wave_strength: float = 0.04
@export_range(1,64) var wave_spread_updates: int =0

@export var surface_line_thickness: float= 1.0
@export var surface_color: Color= Color(0.549, 0.937, 0.933, 0.886)
@export var water_fill_color: Color =Color(0.169, 0.698, 0.898, 0.863)
...
func update_physics(delta:float) -> void:
	for i in range(segment_count):
		...
		var _acceleration: float= -water_restoring_force *displacement - segment_data[i]["velocity"]* wave_energy_loss
	
	for updates in range (wave_spread_updates):
		for i in range(segment_count): 
			if i>0:
				segment_data[i]["wave_to_left"] = (segment_data[i]["height"]- segment_data[i-1]["height"])
				segment_data[i-1]["velocity"] += segment_data[i]["wave_to_left"]*delta*water_physics_speed
			if i< segment_count-1:
				segment_data[i]["wave_to_right"] = (segment_data[i]["height"] -segment_data[i+1]["height"])
...
func update_visuals() -> void:
	...
	for i in range (segment_count):
		points.append(Vector2(i*segment_width,segment_data[i]["height"] as float))
...
func splash(splash_pos: Vector2, splash_velocity:float)-> void:
	...
	var segment_width: float= water_size.x/ (segment_count-1)
	var index: int= int(clamp(local_x_pos/segment_width,0, segment_count -1)as float)
...
func _on_body_entered(body: Node2D) -> void:
	if body is MainCharacter:
		splash(player.global_position, -player.velocity.y * player_splash_multiplier)
